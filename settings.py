...
class SettingsDialog(wx.Dialog):
    ...
    def __init__(self, parent):
        ...
        self.run_drc_setting = wx.CheckBox(
            self,
            id=wx.ID_ANY,
            label="Run DRC before export",
            pos=wx.DefaultPosition,
            size=wx.DefaultSize,
            style=0,
            name="gerber_run_drc",
        ...
        self.run_drc_setting.SetToolTip(
            wx.ToolTip("Run design rule check before generating fabrication files")
        )

        self.run_drc_image = wx.StaticBitmap(
            self,
            wx.ID_ANY,
            loadBitmapScaled("fill-zones.png", self.parent.scale_factor, static=True),
            wx.DefaultPosition,
        ...
        self.run_drc_setting.Bind(wx.EVT_CHECKBOX, self.update_settings)

        run_drc_sizer = wx.BoxSizer(wx.HORIZONTAL)
        run_drc_sizer.Add(self.run_drc_image, 10, wx.ALL | wx.EXPAND, 5)
        run_drc_sizer.Add(self.run_drc_setting, 100, wx.ALL | wx.EXPAND, 5)
        ...
        layout.Add(run_drc_sizer, 0, wx.ALL | wx.EXPAND, 5)
    ...
    def update_run_drc(self, run):
        """Update settings dialog according to the settings."""
        self.run_drc_setting.SetValue(run)
        if run:
            self.run_drc_setting.SetLabel("Run DRC before export")
        else:
            self.run_drc_setting.SetLabel("Skip DRC before export")
    ...
    def load_settings(self):
        ...
        self.update_run_drc(
            self.parent.settings.get("gerber", {}).get("run_drc", True)
        )
